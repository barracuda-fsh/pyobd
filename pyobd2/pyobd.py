...
class MyApp(wx.App):
    ...
    class sensorProducer(threading.Thread):
        ...
        def initCommunication(self):
            ...
            self.connection = obd_io.OBDConnection(
                self.portName,
                self._notify_window,
                self.baudrate,
                self.SERTIMEOUT,
            ...
            if (
                self.connection.connection.status() != "Car Connected"
                ...
                if curstate == 0:  # show status tab
                    ...
                    try:
                        if misfire_cylinder_supported:
                            r = self.connection.connection.query(
                                core.commands.MONITOR_MISFIRE_CYLINDER_1
                            )
                            result = r.value.MISFIRE_COUNT
                            if not result.is_null():
                                wx.PostEvent(
                                    self._notify_window,
                                    TestEvent([15, 2, str(result.value)]),
                                )
                            else:
                                wx.PostEvent(
                                    self._notify_window,
                                    TestEvent([15, 2, "Misfire count wasn't reported"]),
                                )
                            r = self.connection.connection.query(
                                core.commands.MONITOR_MISFIRE_CYLINDER_2
                            )
                            result = r.value.MISFIRE_COUNT
                            if not result.is_null():
                                wx.PostEvent(
                                    self._notify_window,
                                    TestEvent([16, 2, str(result.value)]),
                                )
                            else:
                                wx.PostEvent(
                                    self._notify_window,
                                    TestEvent([16, 2, "Misfire count wasn't reported"]),
                                )
                            r = self.connection.connection.query(
                                core.commands.MONITOR_MISFIRE_CYLINDER_3
                            )
                            result = r.value.MISFIRE_COUNT
                            if not result.is_null():
                                wx.PostEvent(
                                    self._notify_window,
                                    TestEvent([17, 2, str(result.value)]),
                                )
                            else:
                                wx.PostEvent(
                                    self._notify_window,
                                    TestEvent([17, 2, "Misfire count wasn't reported"]),
                                )
                            r = self.connection.connection.query(
                                core.commands.MONITOR_MISFIRE_CYLINDER_4
                            )
                            result = r.value.MISFIRE_COUNT
                            if not result.is_null():
                                wx.PostEvent(
                                    self._notify_window,
                                    TestEvent([18, 2, str(result.value)]),
                                )
                            else:
                                wx.PostEvent(
                                    self._notify_window,
                                    TestEvent([18, 2, "Misfire count wasn't reported"]),
                                )
                    except:
                    ...
                    if first_time_sensors:
                        ...
                        for command in core.commands[1]:
                            if command:
                                if command.command not in (
                                    ...
                                    if s.value is None:
                                        ...
                                        wx.PostEvent(
                                            self._notify_window,
                                            ResultEvent([counter, 2, str(s.value)]),
                                        )
                                        counter = counter + 1
                        # s = self.connection.connection.query(core.commands.ELM_VOLTAGE)
                        # sensor_list.append([core.commands.ELM_VOLTAGE, core.commands.ELM_VOLTAGE.desc, str(s.value)])
                        # wx.PostEvent(self._notify_window, InsertSensorRowEvent(counter))
                        # wx.PostEvent(self._notify_window, ResultEvent([counter, 0, str(core.commands.ELM_VOLTAGE.command)]))
                        # wx.PostEvent(self._notify_window, ResultEvent([counter, 1, str(core.commands.ELM_VOLTAGE.desc)]))
                        ...
                        for sens in sensor_list:
                            ...
                            wx.PostEvent(
                                self._notify_window,
                                ResultEvent([counter, 0, str(sens[0].command)]),
                            )
                            wx.PostEvent(
                                self._notify_window,
                                ResultEvent([counter, 1, str(sens[1])]),
                            )
                            wx.PostEvent(
                                self._notify_window,
                                ResultEvent([counter, 2, str(s.value)]),
                    ...
                    if self._notify_window.ThreadControl == 1:  # clear DTC
                        r = self.connection.connection.query(core.commands["CLEAR_DTC"])
                    ...
                    if first_time_freezeframe:
                        ...
                        for sens in freezeframe_list:
                            wx.PostEvent(
                                self._notify_window,
                                FreezeframeResultEvent([counter, 0, str(sens[0])]),
                            )
                            wx.PostEvent(
                                self._notify_window,
                                FreezeframeResultEvent([counter, 1, str(sens[1])]),
                            )
                            wx.PostEvent(
                                self._notify_window,
                                FreezeframeResultEvent([counter, 2, str(sens[2])]),
                            )
                            counter = counter + 1
                    ...
                    if first_time_graph:
                        ...
                        graph_commands.append(core.commands.ELM_VOLTAGE)
                        sensor_descriptions = []
                            ...
                            if (prev_command is None) or (
                                ...
                                try:
                                    self.graph_y_vals = np.append(
                                        self.graph_y_vals, float(s.value.magnitude)
                                    )
                                ...
                                if s.value is None:
                                    ...
                                    wx.PostEvent(
                                        self._notify_window,
                                        GraphValueEvent([0, 2, str(s.value)]),
                                    )
                                    try:
                            ...
                            if (prev_command1 is None) or (
                                ...
                                try:
                                    self.graph_y_vals1 = np.append(
                                        self.graph_y_vals1, float(s.value.magnitude)
                                    )
                                except AttributeError:
                                    self.graph_y_vals1 = np.append(
                                ...
                                if s.value is None:
                                    ...
                                    wx.PostEvent(
                                        self._notify_window,
                                        GraphsValueEvent([0, 2, str(s.value)]),
                                    )
                                    try:
                            ...
                            if (prev_command2 is None) or (
                                ...
                                try:
                                    self.graph_y_vals2 = np.append(
                                        self.graph_y_vals2, float(s.value.magnitude)
                                    )
                                except AttributeError:
                                    self.graph_y_vals2 = np.append(
                                ...
                                if s.value is None:
                                    ...
                                    wx.PostEvent(
                                        self._notify_window,
                                        GraphsValueEvent([1, 2, str(s.value)]),
                                    )
                                    try:
                            ...
                            if (prev_command3 is None) or (
                                ...
                                try:
                                    self.graph_y_vals3 = np.append(
                                        self.graph_y_vals3, float(s.value.magnitude)
                                    )
                                except AttributeError:
                                    self.graph_y_vals3 = np.append(
                                ...
                                if s.value is None:
                                    ...
                                    wx.PostEvent(
                                        self._notify_window,
                                        GraphsValueEvent([2, 2, str(s.value)]),
                                    )
                                    try:
                            ...
                            if (prev_command4 is None) or (
                                ...
                                try:
                                    self.graph_y_vals4 = np.append(
                                        self.graph_y_vals4, float(s.value.magnitude)
                                    )
                                except AttributeError:
                                    self.graph_y_vals4 = np.append(
                                ...
                                if s.value is None:
                                    ...
                                    wx.PostEvent(
                                        self._notify_window,
                                        GraphsValueEvent([3, 2, str(s.value)]),
                                    )
                                    try:
    ...
    def build_graph_page(self):
        ...
        self.graph_list_ctrl = self.MyListCtrl(
            self.graph_panel,
            tID,
            pos=wx.Point(0, HOFFSET_LIST),
            style=wx.LC_REPORT | wx.SUNKEN_BORDER | wx.LC_HRULES | wx.LC_SINGLE_SEL,
    ...
    def build_graphs_page(self):
        ...
        self.graphs_list_ctrl = self.MyListCtrl(
            self.graphs_panel,
            tID,
            pos=wx.Point(0, HOFFSET_LIST),
            style=wx.LC_REPORT | wx.SUNKEN_BORDER | wx.LC_HRULES | wx.LC_SINGLE_SEL,
    ...
    def build_freezeframe_page(self):
        ...
        self.freezeframe = self.MyListCtrl(
    ...
    def OnInit(self):
        ...
        EVT_RESULT(
            self, self.GetSelectionGraphComboBox, EVT_COMBOBOXGRAPH_GETSELECTION_ID
        )
        EVT_RESULT(
            self, self.GetSelectionGraphsComboBox, EVT_COMBOBOXGRAPHS_GETSELECTION_ID
        )
        EVT_RESULT(
            self, self.SetSelectionGraphComboBox, EVT_COMBOBOXGRAPH_SETSELECTION_ID
        )
        EVT_RESULT(
            self, self.SetSelectionGraphsComboBox, EVT_COMBOBOXGRAPHS_SETSELECTION_ID
        )
...
"""
    ...
    def CodeLookup(self, e=None):
        ...
        proot = root
        # tree.AppendItem(root,"Powertrain (P) Codes")
        codes = pcodes.keys()
        codes = sorted(codes)
        group = ""
        for c in codes:
